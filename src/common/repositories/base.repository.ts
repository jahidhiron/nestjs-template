...
export class BaseRepository<T extends ObjectLiteral> {
  ...
  //   return handle(
  //     async () => {
  //       const { q, searchBy, query, page = 1, limit = 10, sortBy, relations, select } = params;
  ...
  //           /* skip if already joined (avoid duplicates) */
  //           if (
  //             joinedAliases[path] ||
  ...
  //       // --- WHERE CLAUSES ---
  //       // const applyConditions = (obj: Record<string, any>, parentAlias = this.alias): void => {
  ...
  //       //       applyConditions(value, relationAlias);
  //       //     } else {
  ...
  //           const field = String(rawField); // ✅ Ensure string type
  ...
  //       //     qb.addOrderBy(`${this.alias}.${String(s.whom)}`, order);
  //       //   });
  //       // } else {
  ...
  //           /* Handle nested paths like "profile.id" */
  //           if (field.includes('.')) {
  ...
  //             /* Join relation if not already joined */
  //             if (
  //               !qb.expressionMap.joinAttributes.some(
  //                 (j) =>
  //                   j.alias?.name === relationAlias || j.relation?.propertyPath === relationPath,
  //               )
  //             ) {
  ...
  //   return handle(
  //     async () => {
  //       const { q, searchBy, query, sortBy, relations, select } = params;
